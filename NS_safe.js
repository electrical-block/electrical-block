...
function NS_move(e) {
    if (move_safe_data.flag == 1/**確認有再移動物件 */) {//物件pin磁力

        ////////////////////////////////////////////////////////////////////////////////////////////////////////////
        var a = cli2svg(e)//取得滑鼠SVG_XY
        var main_element_all = document.getElementById(move_safe_data.id).getElementsByClassName('pin')//將主物件pin陣列儲存
            ...
            if (Array.from(main_element_all).indexOf(from_element) == -1) {//判斷是否為主物件pin(只通過非主物件pin)
                ...
                Array.from(main_element_all).forEach(main_element => {//主物件pin_forEach
                    ...
                    if (L < length) {//距離過近
                        ...
                        safe.setAttribute('transform', "translate(" + x + "," + y + ")")
                    }
                }
                )
            }
